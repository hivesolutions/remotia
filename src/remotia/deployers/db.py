...
def mysql_dump(
    ssh, database="master", path=None, host="127.0.0.1", username=None, password=None
):
...
def mysql_open(ssh, address):
    common.cmd(
        ssh, 'sed -i "s/bind-address.*/bind-address=%s/g" /etc/mysql/my.cnf' % address
    )
...
def mysql_exec(ssh, command, host="127.0.0.1", username=None, password=None):
    ...
    ) or common.cmd(ssh, 'mysql -e "%s"' % command)
